...
4. Stages the released `.war` into a minimal `scratch` image (see `rfc-connector/Dockerfile.scanner`) and pushes it to the internal InfoSec Google Artifact Registry so it can be picked up by CVE scanning. This image is not a supported RFC runtime distribution — see the SAP JCo licensing note in [rfc-connector/README.md](rfc-connector/README.md).
